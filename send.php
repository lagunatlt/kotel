...
$mail->SMTPSecure = 'ssl';                            
$mail->Port = 465;
$mail->setFrom('avtopodborstas@yandex.ru', 'СТИРКА'); // Ваш Email, Имя
...
//if(!in_array($ipAddr, $badIP) && empty($spam) && empty($spam1) && !empty($name)) { // если не заполнено скрытое поле и если IP-адрес не находится в нашем чёрном списке
    ...
    // если всё ок - отправляем письмо
            ...
            $mail->isHTML(true); 
            $mail->Subject = "Заявка $sub СТИРКА $today"; // Заголовок письма
            $mail->Body =  "<p>Необходима консультация со специалистом $name123</p>
                            <p>к/т <b>$name</b></p>
                            <hr/>
                            <p><i>Сообщение отправлено с сайта 'Стирка и Глажка'</i></p>"; // Текст письма
//}
